...
class Compressor:
    def __init__(self, users, items, interactions, store_dir, state, user_id_col, item_id_col, history_col):
        self.users = users
        self.items = items
    ...
    def compress(self):
        ...
        self.users = self.users[self.users[self.user_id_col].isin(user_set)].drop_duplicates(subset=[self.user_id_col])
